...
test(void)
{
...

    {
      float *image_map = qoMapMemory(t_device, image_mem,
                                     0, 32 * sizeof(float) * 4, 0);
      for (unsigned i = 0; i < 32; i++) {
        image_map[i * 4] = (float)i;
        image_map[i * 4 + 1] = (float)i * 2;
        image_map[i * 4 + 2] = (float)i * 4;
        image_map[i * 4 + 3] = (float)i * 8;
      }
    }
    ...
    /*
    for (unsigned i = 0; i < 32; i++)
        printf("0x%04x, 0x%04x, 0x%04x, 0x%04x\n", data->result[i][0], data->result[i][1], data->result[i][2], data->result[i][3]);
    */

    const uint16_t expected_values[32][4] = {
      { 0x0000, 0x0000, 0x0000, 0x0000 },
      { 0x3800, 0x3c00, 0x4000, 0x4400 },
      { 0x3e00, 0x4200, 0x4600, 0x4a00 },
      { 0x4100, 0x4500, 0x4900, 0x4d00 },
      { 0x4300, 0x4700, 0x4b00, 0x4f00 },
      { 0x4480, 0x4880, 0x4c80, 0x5080 },
      { 0x4580, 0x4980, 0x4d80, 0x5180 },
      { 0x4680, 0x4a80, 0x4e80, 0x5280 },
      { 0x4780, 0x4b80, 0x4f80, 0x5380 },
      { 0x4840, 0x4c40, 0x5040, 0x5440 },
      { 0x48c0, 0x4cc0, 0x50c0, 0x54c0 },
      { 0x4940, 0x4d40, 0x5140, 0x5540 },
      { 0x49c0, 0x4dc0, 0x51c0, 0x55c0 },
      { 0x4a40, 0x4e40, 0x5240, 0x5640 },
      { 0x4ac0, 0x4ec0, 0x52c0, 0x56c0 },
      { 0x4b40, 0x4f40, 0x5340, 0x5740 },
      { 0x4bc0, 0x4fc0, 0x53c0, 0x57c0 },
      { 0x4c20, 0x5020, 0x5420, 0x5820 },
      { 0x4c60, 0x5060, 0x5460, 0x5860 },
      { 0x4ca0, 0x50a0, 0x54a0, 0x58a0 },
      { 0x4ce0, 0x50e0, 0x54e0, 0x58e0 },
      { 0x4d20, 0x5120, 0x5520, 0x5920 },
      { 0x4d60, 0x5160, 0x5560, 0x5960 },
      { 0x4da0, 0x51a0, 0x55a0, 0x59a0 },
      { 0x4de0, 0x51e0, 0x55e0, 0x59e0 },
      { 0x4e20, 0x5220, 0x5620, 0x5a20 },
      { 0x4e60, 0x5260, 0x5660, 0x5a60 },
      { 0x4ea0, 0x52a0, 0x56a0, 0x5aa0 },
      { 0x4ee0, 0x52e0, 0x56e0, 0x5ae0 },
      { 0x4f20, 0x5320, 0x5720, 0x5b20 },
      { 0x4f60, 0x5360, 0x5760, 0x5b60 },
      { 0x4fa0, 0x53a0, 0x57a0, 0x5ba0 },
    };
    for (unsigned i = 0; i < 32; i++)
        for (unsigned j = 0; j < 4; j++)
            t_assert(data->result[i][j] == expected_values[i][j]);

    t_pass();
}

test_define {
    .name = "bug.gitlab-13149",
    .start = test,
    .no_image = true,
};
